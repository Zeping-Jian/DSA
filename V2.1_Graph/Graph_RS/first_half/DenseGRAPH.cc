...
public:
    adjIterator(const DenseGRAPH &G, int v) : G(G), v(v), i(-1) {
        // cout << "DenseGRAPH::adjIterator Construct v=" << v << endl;
    }
    ...
    int nxt() {
        // cout << "DenseGRAPH::adjIterator nxt i=" << i << ", G_V=" << G.V() << endl;
        for (i++; i < G.V(); i++) {
            // cout << "DenseGRAPH::adjIterator nxt in i=" << i << ",v=0" << v << ", G.adj[v][i]=" << G.adj[v][i] << endl;
    ...
    bool end() {
        // cout << "DenseGRAPH::adjIterator i=" << i << ", G_V=" << G.V() << endl;
        return i >= G.V();
    }
};
